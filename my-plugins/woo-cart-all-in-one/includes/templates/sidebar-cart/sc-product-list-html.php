	...
	if ( $product && $product->exists() && $cart_item['quantity'] > 0 && apply_filters( 'woocommerce_cart_item_visible', true, $cart_item, $cart_item_key ) ) {
		...
		<li class="vi-wcaio-sidebar-cart-pd-wrap" data-cart_item_key="<?php echo esc_attr( $cart_item_key ); ?>" data-product_id="<?php echo esc_attr( $product_id ); ?>">
			...
			<div class="vi-wcaio-sidebar-cart-pd-info-wrap">
				...
				<?php
				if ( ! $product_permalink ) {
					echo wp_kses_post( apply_filters( 'woocommerce_cart_item_name', sprintf( '<div class="notd-sku-side-cart">%s</div>', $product->get_sku() ), $cart_item, $cart_item_key ) );
				} else {
					echo wp_kses_post( apply_filters( 'woocommerce_cart_item_name', sprintf( '<a href="%s" class="notd-sku-side-cart">%s</a>', esc_url( $product_permalink ), $product->get_sku() ), $cart_item, $cart_item_key ) );
				}
				?>
				...
				<div class="vi-wcaio-sidebar-cart-pd-desc">
					...
					if ( $product->is_sold_individually() ) {
						echo apply_filters( 'vi_wcaio_mini_cart_pd_qty',
							sprintf( '<div class="vi-wcaio-sidebar-cart-pd-quantity vi-wcaio-hidden" id="vi_wcaio_qty"><input type="hidden" name="viwcaio_cart[%s][qty]" value="1"></div>', $cart_item_key ),
