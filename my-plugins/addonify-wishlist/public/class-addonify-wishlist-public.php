	...
	public function init_actions() {
		...
		if (
			...
			if ( $this->add_to_cart( $product_id ) ) {
				...
				$notice = __( "{$product->get_title()} [:en]is added to the cart.[:ua]додано до кошика.[:ru]обавлен в корзину.[:]", 'addonify-wishlist' );
				...
				if ( (int) addonify_wishlist_get_option( 'remove_from_wishlist_if_added_to_cart' ) == 1 ) {
					...
					if ( $this->remove_from_wishlist( $product_id ) ) {
						...
						$notice .= '<br>' . __( "{$product->get_title()} [:en]has been removed from the wishlist.[:ua]видалено зі списку бажань.[:ru]удален из списка желаний.[:]", 'addonify-wishlist' );
					} else {
