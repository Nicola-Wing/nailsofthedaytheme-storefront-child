					...
					foreach ( $wishlist_product_ids as $product_id ) {
						...
						<tr data-product_row="addonify-wishlist-table-product-row-<?php echo esc_attr( $product_id ); ?>">
							...
							<td class="name">
                                ...
                                <br>
                                <a class="sku" href="<?php echo esc_url( $product->get_permalink() ); ?>">
							...
							<td class="stock">
								...
								if ( $product->get_stock_status() ) {
									switch ( $product->get_stock_status() ) {
										case 'instock':
											echo esc_html__( '[:en]In Stock[:ua]В наявності[:ru]В наличии[:]', 'addonify-wishlist' );
											break;
										case 'outofstock':
											echo esc_html__( '[:en]Out of Stock[:ua]Немає в наявності[:ru]Нет в наличии[:]', 'addonify-wishlist' );
											break;
										default:
											break;
									}
